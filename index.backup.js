...
import { defaultInputBundleOptions, serverRollup } from './rollup.js';

/**
 * @typedef {import('rollup').RollupOptions} RollupOptions
 * @typedef {import('.').AdapterRollupOptions} AdapterRollupOptions
 */
...
export default function ({
	...
	bundleOptions = defaultInputBundleOptions,
...
If you want to suppress this error, set allowReservedSwaRoutes to true in your adapter options.
			...
			await serverRollup(builder, outputDir, tmpDir, bundleOptions.server, debug);

			const staticDir = customStaticDir || join(outputDir, 'static');
			const apiDir = customApiDir || join(outputDir, 'server');
			const functionDir = join(apiDir, 'sk_render');
			const entry = `${tmpDir}/entry.js`;
			builder.log.minor(`Publishing to "${outputDir}"`);
			...
			builder.log(`Copying files from ${files} to ${tmpDir}, relativePath: ${relativePath}`);
			...
			await esbuild.build(default_esbuild_options);
